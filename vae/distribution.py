...
class LogisticNormal(Distribution):
    ...
    @overrides
    def set_priors(self):
        alpha = self._alpha * torch.ones((self.latent_dim, 1))
        log_alpha = alpha.log()

        prior_mean = (log_alpha.transpose(0, 1) - torch.mean(log_alpha, 1)).transpose(0, 1)
        prior_var = (((1 / alpha) * (1 - (2.0 / self.latent_dim))).transpose(0, 1) +
                    (1.0 / (self.latent_dim**2) * torch.sum(1 / alpha, 1))).transpose(0, 1)
    ...
    def compute_KLD(self, posteriors, priors, kl_weight=1.0):
        ...
        prior_var = priors['var'].to(posterior_logvar.device)
        prior_mean = priors['mean'].to(prior_mean.device)
        var_div = posterior_logvar.exp() / prior_var
        diff = posterior_mean - prior_mean
        diff_term = diff * diff / prior_var
        logvar_div = prior_var.log() - posterior_logvar
        neg_kl_divergence = (var_div + diff_term + logvar_div).sum(dim=1) - self.latent_dim
