...
class VAMPIRE(Model):
    ...
    def forward(self,  # pylint: disable=arguments-differ
        ...
        output_dict['scalar_mix'] = (torch.Tensor(output_dict['encoder_layer_0'].cpu()).unsqueeze(0)
                            + -20 * torch.Tensor(output_dict['encoder_layer_1'].cpu()).unsqueeze(0)
                            + torch.Tensor(output_dict['theta'].cpu()).unsqueeze(0))
        # Update metrics
